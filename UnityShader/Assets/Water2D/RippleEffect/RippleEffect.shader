...
Shader "Customer/ShaderEffect/RippleEffect"
{
    Properties
    {
        ...
        _WaveSpeed("Wave Speed", Range(-10, 10)) = 0.3
        _ReflectionStrength("反射 强度", Range(0, 10)) = 1
        _ReflectionColor("反射 颜色", Color) = (1,1,1,1)
        _RefractionStrength("折射 强度", Range(0, 10)) = 0.5
        _Aspect("Aspect", Float) = 1
        [HideInInspector]_WaveDuration("Wave Duration",Float) = 2.0
        _WaveUVDistance("WaveUV Distance",Float) = 0.5
    }
        ...
        CGPROGRAM
            ...
            fixed4 SpriteFrag(v2f IN) : SV_Target
            {
				...
				for(int i = 0; i< _WaveCenters_Num; i++)
				{
                    ...
                    waveOffset1 *= (1 - saturate(fPlayTime / _WaveDuration)) * (1 - saturate(dis / _WaveUVDistance));
				...
				float2 noiseOffset = SamplerFromNoise(IN.texcoord1 + timer * float2(_NoiseSpeedX, _NoiseSpeedY));
