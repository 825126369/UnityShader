...
namespace UnityEngine.UI
{
	...
	public class CenterView : ScrollRect, IPointerUpHandler,IPage
    {
        ...
        public CenterView Init()
        {
            ...
            oriCellY = m_centerItem.localPosition.y;
        ...
        {
            //遍历所有的，判断离哪个最近.
            RectTransform centerRect = null;
            foreach (RectTransform child in content)
            {
                var posX = child.anchoredPosition.x + content.anchoredPosition.x;
                float offset = Mathf.Sin((child.sizeDelta.x * 4f - Mathf.Abs(posX)) / child.sizeDelta.x / 4f) * maxScale;
                offset = Mathf.Min(maxScale, offset);
                float sc =Mathf.Abs(offset);
            ...
            for (int i = 0; i < m_totalPage + 1; i++)
            {
                ...
                //if (centerRect != null)
                //{
                //    float offset = comp.rect.localScale.x;
                //    float offsetY = 0;
                //    float rotateZ = offset;
                //    if (comp.rect.anchoredPosition.x < centerRect.anchoredPosition.x)
                //    {
                //        rotateZ =  offset;
                //        offsetY = -offset;
                //    }
                //    else if (comp.rect.anchoredPosition.x > centerRect.anchoredPosition.x)
                //    {
                //        rotateZ = -offset;
                //        offsetY = -offset;
                //    }
                //    comp.rect.localRotation = Quaternion.Euler(new Vector3(0, 0, rotateZ));
                //    comp.rect.localPosition = new Vector3(v.x, oriCellY + offsetY, v.z);
                //}
                //else
                //{
                //    comp.rect.transform.localPosition = v;
