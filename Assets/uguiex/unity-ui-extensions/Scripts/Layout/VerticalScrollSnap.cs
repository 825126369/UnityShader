...
namespace UnityEngine.UI.Extensions
{
    ...
    public class VerticalScrollSnap : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
    {
        ...
        private LuaFunction _OnPageChanged;

        /// <summary>
        /// 滑动结束时，页面变化的回调
        /// </summary>
        /// <param name="callBack"></param>
        public void RegisterOnScrollEnd(LuaFunction callBack)
        {
        ...
        void Awake()
        {
            ...
            Debug.Log("_currentScreen :" + _currentScreen);


            _scroll_rect.verticalNormalizedPosition = (float)(_screens - _currentScreen - 1) / (float)(_screens - 1);
        ...
        public void GoToScreen(int screenIndex)
        {
            ...
            if (screenIndex < _screens && screenIndex >= 0 && _positions != null)
            {
                _lerp = true;
                var index = screenIndex;// screenIndex; //_screens - screenIndex - 1
                _lerp_target = _positions[index];
        ...
        private void ChangeBulletsInfo(int currentScreen)
        {
            if (Pagination)
                ...
                {
                    Transform child = Pagination.transform.GetChild(i);
                    child.GetComponent<Toggle>().isOn = (currentScreen == i)
                        ? true
                        : false;
                }
            if (_OnPageChanged != null && LuaManager.Instance.Lua != null)
            {
                _OnPageChanged.Call(_currentScreen);
            }
        }
        ...
        public void DistributePages()
        {
            ...
            int childCnt = _screensContainer.transform.childCount;
            _dimension = (childCnt-1-inactiveGmo)*PageStep + _offset * -1;


            _dimension = Math.Max(0,_dimension);
            ...
            for (int i = 0; i < _screensContainer.transform.childCount; i++)
            {
                ...
                if (temp.activeSelf)
                {
                    ...
                    currentYPosition = _dimension + _offset - (i - inactiveGmo) * PageStep;
                    ...
                    //child.anchoredPosition = new Vector2(0, currentYPosition);
                }
                else
                {
                    inactiveGmo++;
            ...
            if (_screens > 0)
            {
                ...
                {
                    _scroll_rect.verticalNormalizedPosition = (float)(_screens - i - 1) / (float)(_screens - 1);
        ...
        public void AddChild(GameObject GO)
        {
            ...
            _scroll_rect.verticalNormalizedPosition = (float)(_screens - _currentScreen - 1) / (_screens - 1);
        }
