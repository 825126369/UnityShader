...
namespace EmojiUI
{
	public class EmojiText : Text
	{
		...
		//文本表情管理器
		public EmojiManager Manager
		{
			get
			{
				if (!_EmojiManager && canvas != null)
				{
					_EmojiManager = EmojiManager.Instance;
					// if (_EmojiManager == null)
					// {
					// 	_EmojiManager = GetComponentInParent<EmojiManager>();
					// 	if (_EmojiManager == null)
					// 	{
					// 		_EmojiManager = canvas.gameObject.AddComponent<EmojiManager>();
					// 	}
					// }
		...
		public override float preferredWidth
		{
			...
			{
                float tempWidth = 0;
           		var tempSettings = GetGenerationSettings(Vector2.zero);
                tempWidth = cachedTextGeneratorForLayout.GetPreferredWidth(_outputText, tempSettings) / pixelsPerUnit;
