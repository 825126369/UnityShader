...
namespace EmojiUI
{
	public class EmojiRenderGroup : IEmojiRender
	{
		...
		private class ListenerData
		{
			...
			public void Set(EmojiText text)
			{
				if (text != null)
				{
			...
			public bool Dirty(EmojiText text)
			{
				if (text != null)
				{
					Transform trans = text.transform;
		...
		private class CanvasGraphicGroup
		{
			...
			public EmojiSpriteGraphic graphic;
		...
		private Dictionary<EmojiText, ListenerData> listeners;
		...
		public EmojiRenderGroup(EmojiManager target)
		{
			manager = target;
		}
		...
		public void Dispose()
		{
			...
			if (GraphicTasks != null)
			{
				for (int i = 0; i < GraphicTasks.Count; ++i)
				{
					...
					EmojiSpriteGraphic target = group.graphic;
					if (target != null)
					{
						target.Draw(null);
						target.SetDirtyMask();
		...
		public void Release(Graphic graphic)
		{
			...
			if (graphic is EmojiSpriteGraphic && PoolObj != null)
			{
		...
		void MarkDirtyStaticEmoji()
		{
			...
			{
				if (listeners == null)
					listeners = new Dictionary<EmojiText, ListenerData>();
		...
		Graphic Parse(EmojiText text, IFillData taginfo, List<string> joblist)
		{
			if (taginfo != null)
			{
		...
		Graphic ParsePosAndUV(EmojiText text, int ID, Vector3[] Pos, Vector2[] UV, List<string> joblist)
		{
		...
		void RefreshSubUIMesh(EmojiText text, CanvasGraphicGroup group, EmojiSpriteAsset matchAsset, Vector3[] Pos, Vector2[] UV, List<string> joblist)
		{
			// set mesh
			...
			EmojiSpriteGraphic graphic = group.graphic;
		...
		EmojiSpriteGraphic CreateSpriteRender()
		{
			if (PoolObj == null)
			{
