...
namespace EmojiUI
{
	public class EmojiParser : IParser
	{
		...
		public void RecordTextUpdate(EmojiText text)
		{
			throw new System.NotImplementedException();
		}

		void FillSpriteTag(EmojiText text,StringBuilder stringBuilder, Match match,int matchindex, int atlasId,string atlasTag)
		{

			if (text.Manager != null && text.Manager.CanRendering(atlasId) && text.Manager.CanRendering(atlasTag))
			{
				EmojiSpriteAsset sprAsset;
				...
				if (tagSprites != null && tagSprites.spritegroups.Count > 0)
				{
                    ...
                    //一些情况下spacesize计算为零造成手机崩溃，这里加个保护
                    if (spacesize == 0) {
                        return;
                    }
					...
					if (fillspacecnt > 0)
					{
						...
						int size =  EmojiManager.EmojiSize();
						tempSpriteTag.Size = new Vector2(size, size); //new Vector2(64, 64);//new Vector2(autosize, autosize);//
		...
		public bool ParsetContent(EmojiText text,StringBuilder textfiller, Match data,int matchindex)
		{
