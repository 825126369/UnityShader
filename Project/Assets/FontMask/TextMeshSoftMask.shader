...
Shader "Customer/TextMeshSoftMask" {
	Properties
	{
	...
	SubShader
	{
		...
		CGPROGRAM
			...
			float4 _FontMask_ST[200];
			float4 _SoftClipList[200];
            int nTextCount;
				...
				if(nTextCount > 0)
				{
					...
					for (int i = 0; i < nTextCount; i++)
					{
						float4 inMask = float4( 
							step(_SoftClipList[i].xy, worldPos.xy), 
							step(worldPos.xy, _SoftClipList[i].zw) 
						);
						if (all(inMask))
						{
							fixed2 maskUV0 = (worldPos.xy - _SoftClipList[i].xy) / (_SoftClipList[i].zw - _SoftClipList[i].xy);
							fixed2 maskUV1 = maskUV0 * _FontMask_ST[i].xy + _FontMask_ST[i].zw;
